...
// ─── Config ───────────────────────────────────────────────────────────────────
const SUPABASE_URL = "https://uegbwedkxiimmfaykwxh.supabase.co";
const SERVICE_ROLE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVlZ2J3ZWRreGlpbW1mYXlrd3hoIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MTM1NTI3NCwiZXhwIjoyMDk2OTMxMjc0fQ._G4RCxmqcnm6mYTG1NQXl7S16wpLvEQZnjFDl6tgjow";
...
// Password hashing matching the app's lib/password.ts
...
async function insertBatch(table: string, rows: Record<string, unknown>[]): Promise<boolean> {
  try {
...
async function main() {
  console.log("🌱 Supabase REST API Seeder");
  console.log("=".repeat(40));

  const results: Record<string, number> = {};

  // Define table seed order (User first since Session depends on it)
  const tables: { name: string; tableName: string; data: Record<string, unknown>[]; needsId?: boolean; needsTimestamps?: boolean }[] = [
    { name: "Products", tableName: "Product", data: products, needsId: true, needsTimestamps: true },
    { name: "Services", tableName: "Service", data: services, needsId: true, needsTimestamps: true },
    { name: "Clients", tableName: "Client", data: clients, needsId: true, needsTimestamps: true },
    { name: "Testimonials", tableName: "Testimonial", data: testimonials, needsId: true, needsTimestamps: true },
    { name: "Blogs", tableName: "Blog", data: blogs, needsId: true, needsTimestamps: true },
    { name: "Projects", tableName: "Project", data: projects, needsId: true, needsTimestamps: true },
    { name: "SiteSettings", tableName: "SiteSetting", data: settings, needsId: true, needsTimestamps: false },
    { name: "TeamMembers", tableName: "TeamMember", data: teamMembers, needsId: true, needsTimestamps: true },
    { name: "Milestones", tableName: "Milestone", data: milestones, needsId: true, needsTimestamps: true },
    { name: "Sectors", tableName: "Sector", data: sectors, needsId: true, needsTimestamps: true },
    { name: "Branches", tableName: "Branch", data: branches, needsId: true, needsTimestamps: true },
    { name: "Careers", tableName: "Career", data: careers, needsId: true, needsTimestamps: true },
  ];

  for (const table of tables) {
    // Check if data already exists
    const count = await getCount(table.tableName);
    if (count > 0) {
    ...
    // Enrich records with id, createdAt, updatedAt
    const enriched = table.data.map((row) => ({
      ...row,
      ...(table.needsId ? { id: cuid() } : {}),
      ...(table.needsTimestamps ? { createdAt: now, updatedAt: now } : {}),
    }));

    const success = await insertBatch(table.tableName, enriched);
    const inserted = success ? enriched.length : 0;
    results[table.name] = inserted;

    if (success) {
      console.log(`✅ ${table.name}: ${inserted} inserted`);
    }
  }

  // ─── User (needs password hashing) ─────────────────────────────────────
  {
    const count = await getCount("User");
    if (count > 0) {
      console.log(`⏭️  User: already has ${count} rows, skipping`);
      results["User"] = 0;
    } else {
      ...
      const success = await insertBatch("User", [adminUser]);
      results["User"] = success ? 1 : 0;
      if (success) {
        console.log(`✅ User: 1 inserted (admin@shrivaari.com / admin123)`);
      }
    }
  }
  ...
  for (const [name, count] of Object.entries(results)) {
    ...
    console.log(`${icon} ${name}: ${count} inserted`);
    totalInserted += count;
  }
