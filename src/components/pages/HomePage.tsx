...
export default function HomePage() {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <FadeIn>
              ...
              <Button
                ...
                variant="outline"
                className="border-[#334155] text-[#444444] hover:bg-[#0D1D3A] hover:text-white rounded-md px-6 h-10 font-semibold transition-colors"
              >
      ...
      {/* ─── Stats Counter ─── */}
      <section className="py-14 md:py-20 bg-[#0D1D3A]">
      ...
      <section className="py-14 md:py-20 bg-white">
        <div className="max-w-[1280px] mx-auto px-5 lg:px-8">
          <FadeIn>
            <div className="flex items-end justify-between mb-10">
              ...
              <Button
                ...
                className="hidden md:inline-flex border-[#dddddd] text-[#444444] hover:bg-[#0D1D3A] hover:text-white hover:border-[#334155] rounded-md"
              >
          ...
          {loading ? (
            ...
            <p className="text-[#666666]">No ongoing projects at the moment.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {projects.slice(0, 3).map((p, i) => (
                <FadeIn key={p.id} delay={i * 0.08}>
                  <Card className="bg-white rounded-lg border border-[#dddddd] shadow-sm card-hover h-full">
                    <CardContent className="p-6">
                      <Badge className="bg-[#efefef] text-[#444444] hover:bg-[#E5E7EB] mb-3 rounded text-sm font-semibold">{p.category || 'Ongoing'}</Badge>
                      <h3 className="text-lg font-bold text-[#1A1A2E] mb-2">{p.name}</h3>
                      <p className="text-[#666666] text-sm mb-3 leading-relaxed line-clamp-3">{p.description}</p>
          ...
          <div className="md:hidden mt-6 text-center">
            <Button
              variant="outline"
              onClick={() => navigate('services')}
              className="border-[#dddddd] text-[#444444] rounded-md"
            >
              View All <ArrowRight className="ml-2 w-4 h-4" />
            </Button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {testimonials.map((t, i) => (
                <FadeIn key={t.id} delay={i * 0.08}>
                  <Card className="bg-white rounded-lg border border-[#dddddd] shadow-sm h-full">
                    <CardContent className="p-6">
                      ...
                      <div className="border-t border-[#dddddd] pt-4">
                        ...
                        <p className="text-[#666666] text-sm">{t.designation}{t.designation && t.company ? ', ' : ''}{t.company}</p>
                      </div>
                    </CardContent>
                  </Card>
                </FadeIn>
                ...
                <FadeIn key={b.id} delay={i * 0.08}>
                  <Card className="bg-white rounded-lg border border-[#dddddd] shadow-sm card-hover h-full">
                    {b.coverImageUrl ? (
                      ...
                      <div className="h-40 bg-[#efefef] flex items-center justify-center rounded-t-lg">
                        <span className="text-4xl font-bold text-[#444444]/10">{b.title.charAt(0)}</span>
                      </div>
                    )}
                    <CardContent className="p-6">
                      ...
                      <p className="text-[#666666] text-sm leading-relaxed line-clamp-3">{b.excerpt}</p>
                    </CardContent>
                  </Card>
                </FadeIn>
              ))}
