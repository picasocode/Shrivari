...
export default function AdminPanel({ onClose }: AdminPanelProps) {
  ...
  return (
    ...
    <div className="fixed inset-0 z-[100] bg-white flex">
      {/* Sidebar */}
      <div className="w-56 bg-[#0D1D3A] flex flex-col shrink-0">
...
function SectionWrapper({ title, loading, error, onRetry, onAdd, children }: {
  ...
  return (
    <>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 text-[#444444] animate-spin" /></div>
      ) : error ? (
        <div className="flex flex-col items-center py-20 text-[#666666]">
          ...
          <TableBody>
            {items.map(s => (
              <TableRow key={s.id}>
                ...
                <TableCell className="hidden md:table-cell text-sm text-[#666666]">{s.slug}</TableCell>
          ...
          <TableBody>
            {items.map(c => (
              <TableRow key={c.id}>
                ...
                <TableCell className="hidden lg:table-cell text-sm text-[#666666]">{c.location}</TableCell>
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
          ...
          <TableBody>
            {items.map(t => (
              <TableRow key={t.id}>
                ...
                <TableCell className="hidden md:table-cell text-sm text-[#666666]">{t.company}</TableCell>
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto bg-white rounded-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-[#666666]">Paste a YouTube link to show this testimonial as a video review on the website. Supported: youtube.com/watch?v=, youtu.be/, embed/</p>
            {ytId && (
              <div className="mt-2 rounded-md overflow-hidden border border-[#dddddd]">
          ...
          <TableBody>
            {items.map(b => (
              <TableRow key={b.id}>
                ...
                <TableCell className="hidden md:table-cell text-sm text-[#666666]">{b.author}</TableCell>
        ...
        <Table>
          ...
          <TableBody>
            {items.map(p => (
              <TableRow key={p.id}>
                <TableCell className="font-medium text-sm">{p.name}</TableCell>
                <TableCell className="hidden md:table-cell text-sm text-[#666666]">{p.client}</TableCell>
...
function MessagesSection() {
  ...
  return (
    <SectionWrapper title="Contact Messages" loading={loading} error={error} onRetry={load}>
      {messages.length === 0 ? (
        <p className="text-[#666666] text-center py-12">No messages yet.</p>
      ) : (
        <div className="space-y-4">
          {messages.map(m => (
            <div
              key={m.id}
              className={`bg-white rounded-md border shadow-sm p-5 ${m.read ? 'border-[#dddddd]' : 'border-[#E8751A]/30 bg-[#E8751A]/[0.02]'}`}
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="text-sm text-[#666666]">{new Date(m.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' })}</span>
              </div>
...
function SettingsSection() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 text-[#444444] animate-spin" /></div>
  }
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-md border border-[#dddddd] shadow-sm p-6 space-y-5">
